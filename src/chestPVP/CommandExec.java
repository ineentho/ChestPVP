	...
	public boolean onCommand(CommandSender sender, Command cmd,
		...
		if (cmd.getName().equalsIgnoreCase("start")) {
			...
			if (!plugin.started) {
				...
				w.setPVP(true);
				for (Iterator<Player> i = w.getPlayers().iterator(); i
						.hasNext();) {
					Player p = i.next();
						...
						.scheduleSyncRepeatingTask(plugin, new Runnable() {
							public void run() {
								...
								if (sS.length() == 1)
									sS = "0" + sS;
								if (!plugin.ended)
									Helper.updateStatus("Game in progress <span style='color:white'>("
											+ sM + ":" + sS + ")</span>");
								else
									Helper.updateStatus(plugin.endString
	...
	void event(int y) {
		if (y == 0) {
			...
			final int s = plugin.getServer().getScheduler()
					.scheduleSyncRepeatingTask(plugin, new Runnable() {
						public void run() {
							Location loc = new Location(w, 100, plugin.top - 5,
									100);
							((ExperienceOrb) w.spawn(loc, ExperienceOrb.class))
									.setExperience(20);
						}
					}, 0L, 5L);
			plugin.getServer().getScheduler()
			...
			for (Iterator<Player> i = w.getPlayers().iterator(); i.hasNext();) {
				Player p = i.next();
				Location loc = new Location(p.getWorld(), 100, p.getWorld()
						.getHighestBlockYAt(100, 100), 100);
				p.teleport(loc);
			}
		} else if (y == 3) {
			// Diamonds!
